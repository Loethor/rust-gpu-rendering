// examples/gpu_gradient.rs
//
// Run with: cargo run --example gpu_gradient
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let u = f32(id.x) / f32(config.width);
    let v = f32(id.y) / f32(config.height);

    let index = id.y * config.width + id.x;
    pixels[index] = pack_rgba8(u, v, 0.4);
}
"#;
...
async fn run() {
    ...
    image::save_buffer("gpu_gradient.png", &data, width, height, ColorType::Rgba8)
        .expect("Failed to save gpu_gradient.png");
    println!("Saved gpu_gradient.png");
}
